...
The simulated response variables —-- y and y1 --- were hypothetical versions of log(FRP). Individual samples of y and y1 varied among sites as a function of EI as observed for log(median FRP) (Fig. 1A). 
...
```{r}
# Model matrix for Stan model
X <- model.matrix(~ lei_s1 + del_ei_s + t_scaled, sim_set)
# Prepare set of values for the model to predict to
del_ei <- seq(-1,0, length = 15)
#make relevant del_ei values equal actual achieved reductions
del_ei[7] <- min(sim_set$del_ei_s[sim_set$site == "LSN0001"])
del_ei[14] <- min(sim_set$del_ei_s[sim_set$site == "LIS0001"])
del_ei[12] <- min(sim_set$del_ei_s[sim_set$site == "LSS0001"])
del_ei <- c(del_ei[1:11],min(sim_set$del_ei_s[sim_set$site == "DBS0004"]),del_ei[14:15])
# Make predictions for a reference site, a control site and the 4 experimental 
# sites used in sim_setwith ei set at level at end of experiment
ei <- ei_ts$lei[ei_ts$sitecode %in% c("LYR0007","BRS0015","DBS0004",
                                   "LSN0001","LIS0001","LSS0001") & 
                ei_ts$date == max(ei_ts$date)]
new_X <- model.matrix(~ lei_s1 + del_ei_s + t_scaled,
                      expand.grid(lei_s1 = ei,
                                  del_ei_s = del_ei, 
                                  t_scaled = max(sim_set$t_scaled)))
dataList <- list(N = dim(X)[1],
                 N2 = dim(new_X)[1],
                 K = dim(X)[2],
                 G = length(sitecodes),
                 y = sim_set$y1,
                 site = as.numeric(match(sim_set$site, sitecodes)),
                 X = X, new_X = new_X)

pars_to_sample <- c("beta","gamma", "sigma_e","log_lik","new_y_pred","y_pred")
nChains <- 4
nIter <- 4000
nWarmup <- 2000
sim_set_mod <-stan(file = "X4_site_newX.stan",  #
                    data = dataList, chains = nChains, iter = nIter,
...
matrix[N2,4] new_X; // the matrix for the predicted values
}
...
// The 8 predictors in X are (in order):
// ei, del_ei, rain1, rain365, channel, ei*rain1, and del_ei*rain1
  for (i in 1:N)  {
