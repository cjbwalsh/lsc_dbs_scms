...
S3 describes compilation of specification data for stormwater control measures (SCMs) implemented in the Little Stringybark Creek and Dobsons Creek catchments. The data was structured as a postgreSQL database. The database's constituent tables are stored in an Open Science Framework repository (https://osf.io/57azq/). Spatial tables are stored in the spatial_data component of the repository, and non-spatial tables are stored in the linked data directory of the github repository https://github.com/cjbwalsh/lsc_dbs_scms.  
and the code in this document assumes it is running from a clone of that repository.  

The calculations were conducted in R using a series of functions in the file "code/BACRIfunctions.R".  The code chunk below loads BACRIfunctions.R and the data. It also includes commented-out code illustrating the method for reading the data from the database.
...
#spatial tables needed to be saved as non-rda files lest they cause recursion problems...
### Run this commented-out unzip command if db_sf.zip has not previously been unzipped 
### in your local environment
if(sum(grepl("gpkg",dir(here::here("data")))) == 0)
unzip(here::here("data","db_sf.zip"))
...
Data detailing the specifications of all SCMs in the two catchments (Little Stringybark and Dobsons) were compiled into the postgreSQL database (Fig. S3-1). 

Each catchment was divided at two scales: into (property and road) parcels, and into pipe subcatchments that permit calculation of impervious runoff and fluxes of waters into and out of SCMs at the property, pipe subcatchment and catchment scale. Specifications of tanks, raingardens and downpipe diverters permit modelling of individual SCMs and treatment trains of SCMs that drain to each other.  Dates of changes in impervious coverage, in installation, decommissioning and specifications of SCMs, permit calculation of time series of fluxes at all scales. [*Do we need to de-identify parcel addresses (and therefore remove parcelsMaps from the collection of files)? Also should data not used in the foundation paper (e.g. cost data) be held back?*].  

![Fig. S3-1. Tables and their relationships in database of stormwater control measures in the Little Stringybark and Dobsons Creek catchments. Spatial tables can be identified by their geometry field.](images/lscdbsSCMsStructure_postgres.png) 
  
  
The following description of the constituent tables explains the logic underlying the dataset and its use.  
  
```{r fig1_pipeIDmap, echo = FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.height = 4}
...
#### Fig. S3-2. Maps of 'pipe' subcatchments used to partition the study catchments of A. Little Stringybark Creek (LSC) and B. Dobsons Creek. The pipeID of each subcatchment is indicated in black, and the next pipeID downstream is indicated in red, italic font. The locations of the 5 LSC and 3 DBS flow gauges used in the study are indicated as gray circles.  

### Table subcs (pipe subcatchments)  

The catchments of the two most downstream experimental sites (D8-e and L4-e) are divided into subcatchments (identified by unique pipeIDs, Fig. S3-2) that permit modelling of impervious runoff and SCM performance at a range of scales. Subcatchments were selected:  

1. At the outlet of pipes which are (or potentially could be) intercepted by an SCM before entering the stream;  

1. At segments along streams upstream of a pipe subcatchment outlet;  

1. At the locations of sampling reaches or gauges.  

The field "pipeName" gives meaningful names to each pipeID either by a street name, or the name of an SCM or the code for a sampling site. The field "nextds" identifies the next pipeID downstream (see Fig. S3-2), allowing compilation of all upstream pipeIDs using the function allupstream(). "scarea" is the area of the subcatchment polygon in m^2^, and "carea" is the total upstream catchment area for each subcatchment (also in m^2^).  "trib" is the gauge site that the subcatchment lies in (i.e. L4-e, L1-e, LN-e, LS-E, D4-e or D8-e).  
 
```{r fig2_parcelMap, echo = FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.height = 4}
par(mar = c(0,0,0,0),mfrow = c(1,2))
...
#### Fig. S3-3. Maps of road (green) and property (orange) parcels of A. Little Stringybark Creek (LSC) and B. Dobsons Creek. .  

### Table parcels  

The catchments of the two most downstream experimental sites were also divided into parcels:  

- property parcels, obtained from the government cadastre (land.vic.gov.au), cut to the catchment boundaries of D8-e and L4-e using QGIS's intersect function. Some manual editing of the resulting property parcel layer was required to a) combine parcels on several properties that functioned as single properties, but were mapped as multiple titles, b) split several parcels that had been recently subdivided; and c) split any property parcels that spanned two pipe subcatchments and that had impervious areas that drained to different subcatchments. The last case was rare, and restricted to some larger properties: most properties had a single point of legal discharge to which stormwater runoff was required to drain.

- road parcels, obtained using QGIS's difference function (between the property parcel and the catchment boundary layers) to identify the road reserves of each catchment, as the cadastre has gaps for road reserves. This extracted road reserve layer was manually split (using the split features tool) at intersections, and a unique parcelID was created for each newly created road parcel. 

Property and road parcels were combined into a single spatial layer for each catchment (Fig.  S3-3), and data associated with each parcel in the two layers were combined into the single table, 'parcels'.  *[Yet to decide which fields should be retained for public datasets.  We should probably remove address information, which will require some broader revision for the L4-e catchment because I use addresses as parcelIDs, and subsequently scmIDs.  Whether the JK estimates of imperviousness at the parcel scale are retained on whether they are used in the final manuscript - currently used in Fig. 6.  Currently the parcels table has the following fields: primary key "address" (other fields, nextds, pipeID [link to subcs], streetNo, streetSuff, streetName, streetType, parcelType [property or road], JKroofArea, JKpaveArea, roofAreaCon, roofAreaUncon, paveAreaCon, paveAreaUncon, parcelArea, parcelID, area_m2 and geometry [all areas in m^2^])]*  
  
```{r fig3_iaMap, echo = FALSE, warning=FALSE, message=FALSE, dev = "png", dpi = 300, fig.width = 7, fig.height = 4}
par(mar = c(0,0,0,0),mfrow = c(1,2))
...
#### Fig. S3-4. Maps of impervious areas of A. Little Stringybark Creek (LSC) and B. Dobsons Creek, coloured by drainage connection (see text).  

### Table ia (impervious surfaces)  

Impervious polygons (see S1 for determination) were split by parcel and by pipe subcatchment to permit parcel-wise and sub-catchment-wise estimation of imperviousness. Each resulting polygon was given a unique polygon identifier (polyID), and a parcel identifier (address, matched to the parcels table) and a pipeID (matched to the subcs table).  The conn field denoted drainage connection, where conn = 1 for surfaces draining to sealed stormwater drainage, 0 for surfaces that are informally drained to land.  
...
- '3', the third round of engagement in the Little Stringybark Creek project funded by offering standardised stormwater treatment systems free of charge, with maximum cost set by rounds 1 and 2 [@bos_brown_2015];  

- '4', the fourth round of engagement in the Little Stringybark Creek project funded by ...Darren is going to need to remind me...  
...
The scmProject table lists each "projectID", "address" (matched to the parcels table, except for large public systems that drain more than one property), "round", "installDate" (the date of installation), "pipeID" (matched to table subcs), and "parcelID" (matched to table parcels).  Finally the table also includes three elements of the cost of each project: "costProject", the cost (in Australian dollars) to the project (administered through the University of Melbourne); "costGovt", any government rebates claimable by the landholder for the installed SCM; and "costOwner", any cost born by the owner of the property.  
...
The SCMs table also lists the fields "address" (matched to table parcels), "round" (matched to table scmProjects), "dbName" (a meaningful descriptor for some SCMs), projectID (matched to table scmProjects), pipeID (matched to table subcs), and parcelID (matched to table parcels).  The three types of SCMs have different sets of specification parameters required to model their performance, thus requiring separate specifications tables (downpipe diverters, dds; tanks and raingardens).  
...
`r dim(tanks)[1] - sum(tanks$round %in% c("E","DBS existing SCMs"))` tanks were installed in the two experimental catchments during the study period, in addition to `r sum(tanks$round %in% c("E","DBS existing SCMs"))` existing tanks.  The tanks table lists the specifications required to model their performance:  
...
ft <- flextable::flextable(tanksMetaData)
...
`r dim(raingardens)[1]` raingardens were installed in the two experimental catchments during the study.  The raingardens table lists the specifications required to model their performance:  
...
ft <- flextable::flextable(rgsMetaData)
ft <- flextable::align(ft, align = "left")
flextable::autofit(ft)
```
...
#### Fig. S3-5. The default mean profile of soil hydraulic conductivity used for most raingardens in the Little Stringybark catchment (mtev.default, light blue dots on map), and the measured profiles used for raingardens at the measurement sites (colours of points match the profile curves).
...
#### Fig. S3-6. Experimental catchments showing connected and unconnected impervious areas and stormwater control measures (SCMs), and in B, D-F, connections between SCMs. A. D8-e;  B. Detail of that part of the D8-eand D4-e catchments serviced by stormwater drainage pipes draining to the stream; C. L4-e; D. L1-e; E. LN-e; F. LS-e.  

Calculation of impervious runoff fluxes and EB in each subcatchment thus required adjustment of the database tables at each time of change to portray the distribution and relationships between impervious surfaces and SCMs. A group of related functions (Fig. S3.7) were thus developed to:   
...
![Fig. S3-7. Primary functions in BACRIfunctions.R (and modelfunctions2017.R in the case of tankmodel and gardenmodel) and the flow of calculations to achieve the primary outputs of budget_subc_time_series and EI_subc_time_series.](BACRIfunctions.png) 
...
#### Fig. S3-8. Map of example subcatchment (`r subcs$pipeName[subcs$pipeID == example_pipeID]`), with a black line indicating upstream-downstream relationships between SCMs. (Locations of SCMs are approximate within their correct land parcel.)

We begin by demonstrating the two primary functions for calculating time series of EI variants and water fluxes, before stepping through the sub-functions that they use in their calculations (Fig. S3-7).  

Time series of EI variants and of modelled flows over the length of the study can simply be calculated and plotted using code shown below for Figs. S3_9 and S3_10.  For this small catchment, the EI calculation (Fig. S3_9) took ~20 s, and the water flux calculation (Figs. S3_10) took ~90 s.  Calculations for larger sub-catchments take between 2 (L1-e) and 50 min (L4-e).   

```{r figS3_9, fig.width=6, fig.height=3}
#' Croydon rainfall-runoff data (recompiled version of Croydon.RData used by modelfunctions to calculate the EB index). 
...
plotlEItrends(ei_29$iats, legend = TRUE, "topleft", cex = 0.8)
title(ylab = "Effective imperviousness variant (%)", xlab = "Date", cex.lab = 0.75)
```

#### Fig. S3-9. Time series plot of six effective impervious variants for example subcatchment (`r subcs$pipeName[subcs$pipeID == example_pipeID]`). Variant codes correspond to the EI subscripts used in the main paper.  
...
system.time(budget_29 <- budget_subc_time_series(pipeID = 29, runoffData = lsc_runoff)) #~ 12 min
